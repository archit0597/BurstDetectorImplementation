#ifndef _WEBPAGEPERFORMANCETEST_H_
#define _WEBPAGEPERFORMANCETEST_H_
...
#include "../Dataset/WebPageDataset.h"
#include "../Utils/Mmap.h"
#include "../BurstDetector/Param.h"
#include "../Utils/Burst.h"

class WebpagePerformanceTest {
public:
    WebpagePerformanceTest(WebPageDataset* _webpage): webpage(_webpage) {}
    ~WebpagePerformanceTest() {}
    ...
    void Run(int memory, int flag) {
        ...
        while(index < run_length) {
            if (count_based)
                ground_truth_detector->insertID(webpage->dataset[index].id, index);
            else 
                ground_truth_detector->insertID(webpage->dataset[index].id, webpage->dataset[index].timestamp);
            ++index;
        }
        ...
        if (BURST_SKETCH_FLAG & flag) {
            ...
            while(index < run_length) {
                if (count_based)
                    burst_sketch->insertID(webpage->dataset[index].id, index);
                else 
                    burst_sketch->insertID(webpage->dataset[index].id, webpage->dataset[index].timestamp);
                ++index;
            }
        ...
        if (OPTIMIZE_SKETCH_FLAG & flag) {
            ...
            while (index < run_length) {
                if (count_based)
                    optimized_burst_sketch->insertID(webpage->dataset[index].id, index);
                else 
                    optimized_burst_sketch->insertID(webpage->dataset[index].id, webpage->dataset[index].timestamp);
                ++index;
            }
