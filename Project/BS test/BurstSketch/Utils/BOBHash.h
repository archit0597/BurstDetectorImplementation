...
static const uint32_t primeNum[] = {
...
public:
    // hashing funtion
    static uint32_t BOBHash32(const uint8_t* str, uint32_t length, uint32_t number){
        //declare variables a,b,c,length;
        uint32_t a,b,c;
        c = primeNum[number]; /* here we are storing the previous hash value */
        ...
        c += length;
        switch(length)              /* every switch must be go through */
        {
    ...
    static uint64_t BOBHash64(const uint8_t* str, uint32_t length, uint32_t number){
        uint64_t a,b,c;
        c = primeNum[number];         /* here we are storing the previous hash value */
        a = b = 0x9e3779b97f4a7c13LL;  /* an arbitrary value */
        for ( ; length >= 24 ; length-=24) {
