#ifndef _WEBPAGEANALYSIS_H_
#define _WEBPAGEANALYSIS_H_
...
public:
    WebpageAnalysis (WebPageDataset *_webpage) : webpage(_webpage) {}
    ~WebpageAnalysis() {}
    ...
    void DetectPotentialBursts() {
        ...
        while (index < run_length) {
            items.insert(webpage->dataset[index].id);
            burst_sketch->insertID(webpage->dataset[index].id, index);
            optimized_sketch->insertID(webpage->dataset[index].id, index);
            ground_detector->insertID(webpage->dataset[index].id, index);
            ++index;
        }
    ...
    double Run(int memory, int flag) {
        ...
        while (index < run_length) {
            if (count_based)
                ...
                ground_detector->insertID(webpage->dataset[index].id, webpage->dataset[index].timestamp);
            ++index;
        }
        ...
        if (flag & OPTIMIZE_SKETCH_FLAG) {
            ...
            while (index < run_length) {
                if (count_based)
                    ...
                    optimized_sketch->insertID(webpage->dataset[index].id, webpage->dataset[index].timestamp);
                ++index;
            }
